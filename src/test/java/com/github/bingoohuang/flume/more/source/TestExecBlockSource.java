...
public class TestExecBlockSource {
    ...
    public void testProcess() throws InterruptedException, LifecycleException,
        ...
        for (int i = 0; i < 10; i++) {
            if (i > 0 && random.nextBoolean()) {
                outputStream1.write(simpleDateFormat.format(new Date()).getBytes());
                outputStream1.write(' ');
                ++blockCount;
            } else if (i  == 0) {
                ++blockCount;
        ...
        context.put("capacity", "1000");
        context.put("transactionCapacity", "1000");
        context.put("boundaryRegex", "\\d{4}-\\d{2}-\\d{2}"); // yyyy-MM-dd
        context.put("batchTimeout", "1500");
        ...
        assertThat(realEventCount, is(blockCount));
